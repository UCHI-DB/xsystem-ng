...
public class Symbol {
	...
	private XClass repfn(){
		...
		if(symbolPcts.get(maxSym) > 0.9){
			if(! maxSym.isClass()) return maxSym.toXClass();
				...
				if(!utils.significant(new ArrayList<Double>(totalHist.values()))) return maxSym.toXClass(); // Null hypothesis accepted
				else if(trucatedHist.size() == 1) return new X_SPEC(String.valueOf(trucatedHist.keySet().iterator().next())); // Only one character
				else{
	...
	public XClass representation = repfn();

	public ArrayList<String> symbolStringGenerator = representation.lshDomain();
